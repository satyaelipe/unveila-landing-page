...
export default function LandingPage() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const offsets = sections.map(id => {
        ...
        if (el) {
          return { id, offset: el.getBoundingClientRect().top - 100 };
        }
        return null;
      }).filter(Boolean);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#0a0a1a] to-[#0d0f24] text-white flex flex-col scroll-smooth">
      ...
      {/* Hero Section */}
      <main id="home" className="flex-grow flex flex-col items-center justify-center px-6 text-center opacity-0 translate-y-4 transition-all duration-700 scroll-fade">
      ...
      {/* Why Unveila */}
      <section id="why-unveila" className="scroll-fade scroll-mt-24 bg-gray-100 text-gray-800 py-16 px-4 sm:px-8">
      ...
      <section id="what-we-solve" className="scroll-fade scroll-mt-24 bg-white text-gray-800 py-16 px-4 sm:px-8">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="grid gap-8 sm:grid-cols-2 text-left">
            <div>
              ...
              <p className="text-gray-600">Struggling to track what’s deployed across accounts, regions, and clouds? We help you map and manage your infra in minutes.</p>
            </div>
            <div>
      ...
      {showScrollTop && (
        <button
          className="fixed bottom-6 right-6 bg-blue-600 hover:bg-blue-700 p-3 rounded-full text-white shadow-lg transition-opacity"
